...
class ThreadGuardContainer {
 public:
  ThreadGuardContainer() = default;

  template <class Callable, class... Args>
  void emplace_back(Callable&& callable, Args&&... args) {
    guards_.emplace_back(std::thread{std::forward<Callable>(callable), std::forward<Args>(args)...});
  }

  ThreadGuardContainer(const ThreadGuardContainer&) = delete;
  ThreadGuardContainer& operator=(const ThreadGuardContainer&) = delete;
  ThreadGuardContainer(ThreadGuardContainer&&) = delete;
  ThreadGuardContainer& operator=(ThreadGuardContainer&&) = delete;

 private:
  std::vector<ThreadGuard> guards_;
};
