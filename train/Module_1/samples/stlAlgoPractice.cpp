 ...
 public:
  void generateTestData(int count) {
    ...
    for (int i = 0; i < count; ++i) {
      transactions.emplace_back(customerDist(rng), amountDist(rng), categories[i % categories.size()],
                                timestampDist(rng));
    }
  ...
  void demonstrateSearchAlgorithms() {
    ...
    double traditionalFindTime = measureTime([&]() {
      auto it = std::find_if(transactions.begin(), transactions.end(),
                             [targetCustomer](const CustomerTransaction& t) { return t.customerId == targetCustomer; });
    ...
    double traditionalCountTime = measureTime([&]() {
      electronicsCount = std::count_if(transactions.begin(), transactions.end(),
                                       [](const CustomerTransaction& t) { return t.category == "Electronics"; });
    });

    std::cout << "  find_if execution time: " << std::fixed << std::setprecision(3) << traditionalFindTime << " ms"
              << std::endl;
    ...
    double rangeFindTime = measureTime([&]() {
      auto result = rng::find_if(
          transactions, [targetCustomer](const CustomerTransaction& t) { return t.customerId == targetCustomer; });
    ...
    double rangeCountTime = measureTime([&]() {
      rangeElectronicsCount =
          rng::count_if(transactions, [](const CustomerTransaction& t) { return t.category == "Electronics"; });
    });
    ...
    std::cout << "  Find speedup with ranges: " << (traditionalFindTime > 0 ? traditionalFindTime / rangeFindTime : 1.0)
              << "x" << std::endl;
  ...
  void demonstrateTransformationAlgorithms() {
    ...
    double transformTime = measureTime([&]() {
      std::transform(workingCopy.begin(), workingCopy.end(), adjustedAmounts.begin(), [](const CustomerTransaction& t) {
        // Apply 10% discount
        return t.amount * 0.9;
      });
    });
    ...
    double rangeTransformTime = measureTime([&]() {
      auto discountedView = workingCopy | views::transform([](const CustomerTransaction& t) { return t.amount * 0.9; });
    ...
    auto lazyTransform = workingCopy | views::transform([](const CustomerTransaction& t) { return t.amount * 0.9; }) |
                         views::filter([](double amount) { return amount > 100.0; }) | views::take(5);
  ...
  void demonstrateSortingAlgorithms() {
    ...
    double amountSortTime = measureTime([&]() {
      std::sort(sortByAmount.begin(), sortByAmount.end(),
                [](const CustomerTransaction& a, const CustomerTransaction& b) { return a.amount > b.amount; });
    });

    // Sort by customer ID
    double customerSortTime = measureTime([&]() {
      std::sort(sortByCustomer.begin(), sortByCustomer.end(),
                [](const CustomerTransaction& a, const CustomerTransaction& b) { return a.customerId < b.customerId; });
    });
    ...
    for (int i = 0; i < std::min(5, static_cast<int>(sortByAmount.size())); ++i) {
      ...
      std::cout << "    Customer " << t.customerId << ": $" << std::fixed << std::setprecision(2) << t.amount << " ("
                << t.category << ")" << std::endl;
    }
    ...
    double rangeSortTime = measureTime([&]() {
      rng::sort(rangeSortCopy,
                [](const CustomerTransaction& a, const CustomerTransaction& b) { return a.amount > b.amount; });
    });

    std::cout << "  ranges::sort time: " << rangeSortTime << " ms" << std::endl;
    std::cout << "  Performance comparison: " << (amountSortTime > 0 ? amountSortTime / rangeSortTime : 1.0) << "x"
              << std::endl;
#endif
  }
};
