...
struct SalesData {
  ...
  double pricePerUnit() const { return units > 0 ? revenue / units : 0.0; }
};
...
class AlgorithmPipeline {
 ...
 public:
  ...
  void traditionalAnalysisPipeline() {
    ...
    std::copy_if(salesData.begin(), salesData.end(), std::back_inserter(highRevenueSales),
                 [](const SalesData& s) { return s.revenue > 3000.0; });

    std::cout << "Step 1 - High revenue sales: " << highRevenueSales.size() << " out of " << salesData.size()
              << std::endl;

    // Pipeline Step 2: Transform to calculate price per unit
    std::vector<double> pricesPerUnit;
    std::transform(highRevenueSales.begin(), highRevenueSales.end(), std::back_inserter(pricesPerUnit),
                   [](const SalesData& s) { return s.pricePerUnit(); });

    std::cout << "Step 2 - Price per unit calculated for " << pricesPerUnit.size() << " records" << std::endl;
    ...
    double avgPricePerUnit = std::accumulate(pricesPerUnit.begin(), pricesPerUnit.end(), 0.0) / pricesPerUnit.size();
    ...
    std::cout << "  Total high-revenue sales: $" << std::fixed << std::setprecision(2) << totalRevenue << std::endl;
    ...
    std::for_each(highRevenueSales.begin(), highRevenueSales.end(),
                  [&regionTotals](const SalesData& s) { regionTotals[s.region] += s.revenue; });

    auto topRegion = std::max_element(regionTotals.begin(), regionTotals.end(),
                                      [](const auto& a, const auto& b) { return a.second < b.second; });

    std::cout << "Step 4 - Top performing region: " << topRegion->first << " with $" << topRegion->second << std::endl;
    ...
    for (int i = 0; i < std::min(3, static_cast<int>(sortedSales.size())); ++i) {
      ...
      std::cout << "  " << i + 1 << ". " << s.product << " (" << s.region << ", " << s.month << "): $" << s.revenue
                << std::endl;
    }
  }
  ...
  void modernRangesPipeline() {
    ...
    auto highRevenueAnalysis = salesData | views::filter([](const SalesData& s) { return s.revenue > 3000.0; }) |
    ...
    auto priceAnalysis = highRevenueSales | views::transform([](const SalesData& s) { return s.pricePerUnit(); }) |
    ...
    std::vector<std::string> regions = {"North", "South", "East", "West"};
    for (const auto& region : regions) {
      auto topProductInRegion =
          highRevenueSales | views::filter([&region](const SalesData& s) { return s.region == region; }) |
          views::transform([](const SalesData& s) { return std::make_pair(s.product, s.revenue); });

      if (!topProductInRegion.empty()) {
        auto maxProduct =
            rng::max_element(topProductInRegion, [](const auto& a, const auto& b) { return a.second < b.second; });
    ...
    auto complexPipeline = salesData | views::filter([](const SalesData& s) { return s.units > 20; }) |
  ...
  void performanceComparison() {
    ...
    std::cout << "Traditional STL pipeline: " << std::fixed << std::setprecision(3) << traditionalTime << " ms"
              << std::endl;

#if HAS_RANGES
    // Ranges approach timing
    double rangesTime = measureExecutionTime([this]() {
      auto pipeline = salesData | views::filter([](const SalesData& s) { return s.revenue > 3000.0 && s.units > 15; }) |
