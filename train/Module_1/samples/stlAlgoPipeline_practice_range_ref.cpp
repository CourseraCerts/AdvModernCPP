...
class IsValidTransaction {
 public:
  bool operator()(const Transaction& t) const { return t.amount > 0 && !t.customerId.empty(); }
};

class IsHighValueTransaction {
 public:
  bool operator()(const Transaction& t) const { return t.finalTotal > 500.0; }
};
...
class CalculateDerivedValues {
 public:
  Transaction operator()(Transaction t) const {
    ...
    t.discount = (t.amount > DISCOUNT_THRESHOLD) ? t.amount * DISCOUNT_RATE : 0.0;
...
bool hasValidAmount(const Transaction& t) { return t.amount > 0; }

bool hasValidCustomer(const Transaction& t) { return !t.customerId.empty(); }

double extractFinalTotal(const Transaction& t) { return t.finalTotal; }
...
double addFinalTotal(double sum, const Transaction& t) { return sum + t.finalTotal; }
...
class AlgorithmPipeline {
 ...
 public:
  ...
  void calculateDerivedValues() {
    ...
    transform(validTransactions.begin(), validTransactions.end(), validTransactions.begin(), calculator);
    ...
    if (!validTransactions.empty()) {
      ...
      cout << "Sample: Amount $" << fixed << setprecision(2) << sample.amount << ", Tax $" << sample.taxAmount
           << ", Discount $" << sample.discount << " -> Final $" << sample.finalTotal << endl;
    }
  }
  ...
  void generateStatistics() {
    ...
    double totalRevenue = accumulate(validTransactions.begin(), validTransactions.end(), 0.0, addFinalTotal);
    ...
    auto [minIt, maxIt] =
        minmax_element(validTransactions.begin(), validTransactions.end(),
                       [](const Transaction& a, const Transaction& b) { return a.finalTotal < b.finalTotal; });
    ...
    cout << "Highest Transaction: $" << maxIt->finalTotal << " (Customer: " << maxIt->customerId << ")" << endl;
    cout << "Lowest Transaction: $" << minIt->finalTotal << " (Customer: " << minIt->customerId << ")" << endl;
    cout << "High-Value Transactions (>$500): " << highValueCount << " (" << fixed << setprecision(1)
  ...
  void sortAndRank() {
    ...
    for (size_t i = 0; i < min(5ul, sortedTransactions.size()); ++i) {
      ...
      cout << i + 1 << ". Customer " << t.customerId << " - $" << fixed << setprecision(2) << t.finalTotal << " ("
           << t.region << ")" << endl;
    }

    // Use partial_sort to find top 10 transactions efficiently
    vector<Transaction> topTransactions = validTransactions;
    partial_sort(topTransactions.begin(), topTransactions.begin() + min(10ul, topTransactions.size()),
    ...
    for (const auto& t : sortedTransactions) {
      ...
      if (regionCount < 3) {  // Show top 3 per region
        cout << "  " << regionCount + 1 << ". Customer " << t.customerId << " - $" << fixed << setprecision(2)
             << t.finalTotal << endl;
        regionCount++;
      }
    }
